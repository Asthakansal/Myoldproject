...
public class ReviewRestController {
	...
	@PostMapping("/user/{username}/review/{description}")
	public ResponseEntity<MyDTO> addReviewByUser(@PathVariable String username,@PathVariable String description)
	{
		...
		try {
			...
			//	Review post = new Review(description, LocalDate.now().toString(), 0, 0);
				...
				if(savedReview.getReviewId() != 0)
				{
					...
					UserReviewCreatedResponseDTO dto = UserDTOConvertor.getReviewCreatedDTO(updatedUserWithReview,savedReview);
